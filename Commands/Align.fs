...
let analyzeReference
    ...
    async {
        ...
        return
            ...
            |> Result.map (fun img ->
                printfn $"Image dimensions: {img.Width}x{img.Height}, {img.Channels} channel(s)"

                let frame = Alignment.prepareReference detConfig maxStarsTriangles img
...
let showHelp() =
    ...
    printfn ""
    printfn "Description:"
    printfn "  Railway-oriented pipeline for star detection and image registration."
    printfn "  Pipeline: Load → Detect → Match → Distort → Transform → Save"
    ...
    printfn "                              detect     - Star detection only"
...
let run (args: string array) =
    if args |> Array.exists (fun a -> a = "--help" || a = "-h") then
        ...
        let setupResult = asyncResult {
            ...
            let! refData = analyzeReference refFile detConfig opts.MaxStarsTriangles
            ...
            let pipelineConfigs = buildPipelineConfigs opts.OutputMode opts refData
